...
def add_hard_constraints(prob, assignment_vars, staff_data, shift_data):
    for ca in shift_data['UNIQUE_KEY']:
        ...
        num_staff = 1
        if 'Số lượng cán bộ cần thiết' in shift_row.columns:
            num_staff = int(shift_row['Số lượng cán bộ cần thiết'].squeeze())

        staff_in_shift = [assignment_vars[cb][ca]
                          for cb in staff_data['MS của CÁN BỘ COI THI']
                          if ca in assignment_vars[cb]]
        if staff_in_shift:
            prob += sum(staff_in_shift) == num_staff, f"exact_people_shift_{ca}"

    if 'MS Ca thi' in shift_data.columns:
        ms_ca_groups = shift_data.groupby('MS Ca thi')['UNIQUE_KEY'].apply(list).to_dict()
        for cb in staff_data['MS của CÁN BỘ COI THI']:
    ...
    if 'MS Ca thi' in shift_data.columns and 'Cơ sở' in shift_data.columns:
...
def get_fairness_penalty(prob, assignment_vars, staff_data, shift_data):
    ...
    total_required = 0
    if 'Số lượng cán bộ cần thiết' in shift_data.columns:
        total_required = int(shift_data['Số lượng cán bộ cần thiết'].fillna(1).sum())
    else:
...
def get_same_day_diff_facility_penalty(prob, assignment_vars, staff_data, shift_data):
    ...
    if 'MS Ca thi' not in shift_data.columns or 'Cơ sở' not in shift_data.columns:
        return 0
    ...
    for cb in staff_data['MS của CÁN BỘ COI THI']:
        ...
        for ca in shift_data['UNIQUE_KEY']:
            ...
            ms_ca = str(shift_row.get('MS Ca thi', ''))
            date_str = ms_ca.split('_')[0] if '_' in ms_ca else ms_ca
...
def get_rest_gap_penalty(prob, assignment_vars, staff_data, shift_data):
    ...
    if 'MS Ca thi' not in shift_data.columns:
        return 0

    date_time_map = {}
    for uk, ms_ca in shift_data.set_index('UNIQUE_KEY')['MS Ca thi'].dropna().items():
...
def get_distance_penalty(assignment_vars, staff_data, shift_data):
    ...
    staff_lookup = staff_data.set_index('MS của CÁN BỘ COI THI')
    distance_cost = 0
    ...
    for cb in staff_data['MS của CÁN BỘ COI THI']:
        ...
        for ca in shift_data['UNIQUE_KEY']:
            ...
            if pd.notna(distance):
                distance_cost += config.DISTANCE_WEIGHT * (float(distance)/10.0) * assignment_vars[cb].get(ca, 0)
...
def get_age_penalty(assignment_vars, staff_data, shift_data):
    ...
    ages = pd.to_numeric(staff_data.set_index('MS của CÁN BỘ COI THI')['Tuổi'], errors='coerce').to_dict()
    age_cost = 0
    ...
    for cb, age in ages.items():
        ...
        age_penalty = config.AGE_WEIGHT * (age - 40) / 10
        for ca in shift_data['UNIQUE_KEY']:
            if ca in assignment_vars[cb]:
...
def _staff_distance(staff_row, facility):
    facility_norm = _normalize_facility(facility)
    distance_col = FACILITY_DISTANCE_MAP.get(facility_norm)
    if distance_col and distance_col in staff_row.index:
        try:
            return float(staff_row.get(distance_col, 0) or 0)
        except Exception:
            return 0.0

    if facility_norm == 'Cơ sở 1' and 'Khoảng cách đến CS1' in staff_row.index:
        return float(staff_row.get('Khoảng cách đến CS1', 0) or 0)
    if facility_norm == 'Cơ sở 2' and 'Khoảng cách đến CS2' in staff_row.index:
        return float(staff_row.get('Khoảng cách đến CS2', 0) or 0)
